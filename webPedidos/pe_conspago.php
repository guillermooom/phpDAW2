<HTML>
<HEAD><TITLE>HISTORIAL PAGOS</TITLE></HEAD>
<style>
body{
    font-family:Calibri;
}
...
if(!empty($_POST)){
    ...
    else if(($_POST["f_fin"] < $_POST["f_inicio"])|| $_POST["f_inicio"]=="" || $_POST["f_fin"]==""){
        $realizar=false;
    ...
    if($realizar){
        $vuelta=ordenesPedidas($conn,$fin,$inicio,$nombre);
        foreach($vuelta as $dato){
            dineroGastado($conn,$dato["orderNumber"]);
        }
    }
}
$conn = null;
